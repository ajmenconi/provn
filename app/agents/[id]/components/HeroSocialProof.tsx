...
const PLATFORM_CONFIG: Record<string, { bg: string; short: string }> = {
  'Google':      { bg: '#4285F4', short: 'G' },
  'Zillow':      { bg: '#006AFF', short: 'Z' },
  'Realtor.com': { bg: '#D92228', short: 'R' },
  'Homes.com':   { bg: '#7B2FBE', short: 'H' },
};
  ...
  return (
    ...
    >
      ...
      <div className="flex items-end gap-3">
        <div>
          <span className="font-black text-white tabular-nums leading-none" style={{ fontSize: '64px', lineHeight: 1 }}>
      ...
      <div className="grid grid-cols-2 gap-2">
        {agent.reviewPlatforms.map((p) => {
          const cfg = PLATFORM_CONFIG[p.platform] ?? { bg: '#6b7280', short: p.platform[0] };
          return (
            <div
              key={p.platform}
              className="flex items-center gap-2.5 px-3 py-2.5 rounded-xl"
              style={{ backgroundColor: 'rgba(255,255,255,0.04)', border: '1px solid #2D3148' }}
            >
              <span
                className="w-7 h-7 rounded-lg flex items-center justify-center font-black shrink-0"
                style={{ background: '#1A1D2E', border: `2px solid ${cfg.bg}`, color: cfg.bg, fontSize: '11px' }}
              >
                {cfg.short}
              </span>
              <div>
      ...
      <div
        ...
        style={{ backgroundColor: 'rgba(59,130,246,0.15)', border: '1px solid rgba(147,197,253,0.20)' }}
      >
        <svg className="w-5 h-5 shrink-0" fill="none" viewBox="0 0 24 24" stroke="rgba(147,197,253,0.9)" strokeWidth={2}>
          <path strokeLinecap="round" strokeLinejoin="round" d="M9 12.75 11.25 15 15 9.75m-3-7.036A11.959 11.959 0 0 1 3.598 6 11.99 11.99 0 0 0 3 9.749c0 5.592 3.824 10.29 9 11.623 5.176-1.332 9-6.03 9-11.622 0-1.31-.21-2.571-.598-3.751h-.152c-3.196 0-6.1-1.248-8.25-3.285Z" />
        </svg>
        <div className="flex-1 min-w-0">
          <p className="font-semibold" style={{ fontSize: '13px', color: '#93c5fd' }}>Provn Verified Survey</p>
          <p className="leading-relaxed" style={{ fontSize: '12px', color: '#93c5fd', opacity: 0.7, lineHeight: 1.5 }}>
            Independent · issued 90 days post-close
          </p>
        </div>
        <p className="font-black tabular-nums shrink-0" style={{ fontSize: '24px', color: '#93c5fd' }}>
      ...
      <div className="flex flex-wrap gap-2">
        {agent.reviewKeywordThemes.map((theme) => (
          <span
            ...
            style={{ fontSize: '12px', backgroundColor: 'rgba(255,255,255,0.08)', color: C_INTERP }}
          >
            {theme}
          </span>
        ))}
      ...
      {/* Source */}
      <div className="pt-1 border-t" style={{ borderColor: '#2D3148' }}>
