...
/**
 * /match/buyer/results — Buyer match results page.
 * Reads buyerAnswers from localStorage, scores mock agents, renders top-3 cards.
 */
...
interface MockAgent {
  ...
  reviewScore:           number;
  reviewCount:           number;
  specialties:           string[];  // maps to buyer priorities
  highlights:            string[];  // 3 bullet points shown on card
  locations:             string[];  // cities they serve
  phone:                 string;
}
...
const AGENTS: MockAgent[] = [
  {
    ...
    priceBandLow:          1_000_000,
    priceBandHigh:         3_000_000,
    reviewScore:           4.9,
    reviewCount:           206,
    specialties:           ['Luxury or high-end experience', 'Strong negotiation track record', 'Deep neighborhood knowledge', 'Off-market deal access'],
    highlights:            ['Sells in 11 days avg vs 28-day market median', 'Top 4% career volume — $468M in verified closings', '$103.2% sale-to-list ratio (market avg: 99.4%)'],
  ...
  },
  {
    ...
    brokerage:             'Keller Williams North Bay',
    city:                  'Santa Rosa',
    county:                'Sonoma',
    provnScore:            81,
    provnGrade:            'B+',
    ...
    priceBandLow:          400_000,
    priceBandHigh:         950_000,
    reviewScore:           4.8,
    reviewCount:           118,
    specialties:           ['Experience with first-time buyers', 'Deep neighborhood knowledge', 'Patient and educational style', 'Fast responsive communication'],
    highlights:            ['194 verified closings — 60+ first-time buyer deals', 'Guides buyers through every step, including financing referrals', 'Santa Rosa specialist with 14 years of local transaction data'],
    locations:             ['Santa Rosa', 'Rohnert Park', 'Cotati', 'Windsor', 'Petaluma'],
    phone:                 '(707) 555-0341',
  },
  {
    ...
    priceBandLow:          650_000,
    priceBandHigh:         2_500_000,
    reviewScore:           4.85,
    reviewCount:           159,
    specialties:           ['Deep neighborhood knowledge', 'Investment property knowledge', 'Off-market deal access', 'Strong negotiation track record'],
    highlights:            ['29 off-market buyer deals in the last 3 years', '241 closings across Sonoma and Napa counties', '15-day avg close vs 28-day market — consistent top performer'],
...
function parseBudget(budget: string): [number, number] {
  if (budget === '$3M+') return [3_000_000, Infinity];
  const nums = budget.replace(/\$/g, '').replace(/K/g, '000').replace(/M/g, '000000').split('–').map(s => parseInt(s.replace(/\D/g, ''), 10));
  if (nums.length === 1) return [0, nums[0]];
  return [nums[0], nums[1]];
}

// ── Matching algorithm ─────────────────────────────────────────────────────────
function scoreAgent(agent: MockAgent, answers: BuyerAnswers): number {
  let score = 0;

  // 1. Location match (35 pts)
  const locationMatch = answers.locations.some(loc =>
    agent.locations.some(aLoc => aLoc.toLowerCase().includes(loc.toLowerCase()) || loc.toLowerCase().includes(aLoc.toLowerCase()))
  );
  if (locationMatch) score += 35;
  else if (answers.locations.includes('Other') || answers.locations.length === 0) score += 15;

  // 2. Budget match (25 pts)
  const [buyerLow, buyerHigh] = parseBudget(answers.budget || 'Under $500K');
  const overlap = buyerLow <= agent.priceBandHigh && buyerHigh >= agent.priceBandLow;
  if (overlap) score += 25;
  else {
    const gap = Math.min(Math.abs(buyerHigh - agent.priceBandLow), Math.abs(buyerLow - agent.priceBandHigh));
    if (gap < 200_000) score += 10;
  }
    ...
    <div className="flex flex-col items-center gap-1">
      <svg width={72} height={72} style={{ transform: 'rotate(-90deg)' }}>
        ...
        <text x={cx} y={cy + 1} fill="white" fontSize={14} fontWeight={700}
          textAnchor="middle" dominantBaseline="middle" style={{ transform: 'rotate(90deg)', transformOrigin: `${cx}px ${cy}px` }}>
          {grade}
        </text>
      </svg>
...
function StatPill({ value, label, highlight }: { value: string; label: string; highlight?: boolean }) {
  return (
    <div className="flex flex-col items-center gap-1 px-3 py-3 rounded-xl"
      style={{ background: highlight ? 'rgba(59,130,246,0.08)' : 'rgba(255,255,255,0.04)', border: `1px solid ${highlight ? 'rgba(59,130,246,0.25)' : CARD_BD}` }}>
...
// ── Agent card ─────────────────────────────────────────────────────────────────
function AgentCard({ agent, rank, answers }: { agent: MockAgent; rank: number; answers: BuyerAnswers }) {
  const cfg = RANK_CONFIG[rank];

  // Build highlights: swap in priority-matched reasons if possible
  const priorityHits = answers.priorities.filter(p => agent.specialties.includes(p));
  const matchReasons: string[] = [...agent.highlights];
    ...
    >
      ...
      <div className="p-5">
        ...
        <div className="grid grid-cols-3 gap-2 mb-5">
          <StatPill value={`${agent.avgDaysOnMarket}d`} label="Avg days on market" highlight />
          <StatPill value={`${agent.salePriceToListRatio}%`} label="Sale-to-list ratio" />
          <StatPill value={agent.totalTransactions.toLocaleString()} label="Verified deals" />
        </div>
        ...
        <div className="rounded-xl p-4 mb-4" style={{ background: 'rgba(255,255,255,0.03)', border: `1px solid ${CARD_BD}` }}>
          ...
          <div className="flex flex-col gap-2">
            {matchReasons.map((r, i) => <MatchBullet key={i} text={r} />)}
          </div>
        </div>
        ...
        <div className="flex items-center gap-1.5 mb-5 px-1">
          ...
          <span className="text-xs font-semibold text-white ml-0.5">{agent.reviewScore.toFixed(1)}</span>
...
function ComparisonTable({ agents }: { agents: MockAgent[] }) {
  const rows: { label: string; getValue: (a: MockAgent) => string; better: 'higher' | 'lower' }[] = [
    ...
    { label: 'Verified Closings',    getValue: a => a.totalTransactions.toLocaleString(),       better: 'higher' },
    { label: 'Review Score',         getValue: a => `${a.reviewScore.toFixed(1)} / 5.0`,       better: 'higher' },
    { label: 'Review Count',         getValue: a => a.reviewCount.toLocaleString(),             better: 'higher' },
  ];
  ...
  return (
    <div className="rounded-2xl overflow-hidden" style={{ background: CARD_BG, border: `1px solid ${CARD_BD}` }}>
      ...
      {rows.map((row, ri) => {
        const values = agents.map(a => row.getValue(a));
        const numerics = agents.map(a =>
          ...
          row.label === 'Verified Closings' ? a.totalTransactions :
          row.label === 'Review Score' ? a.reviewScore :
          a.reviewCount
        );
        const best = row.better === 'higher' ? Math.max(...numerics) : Math.min(...numerics);
        return (
          <div key={ri} className="grid px-5 py-3" style={{
            gridTemplateColumns: '1fr repeat(3, 1fr)',
            borderBottom: ri < rows.length - 1 ? `1px solid ${CARD_BD}` : undefined,
          }}>
...
function LoadingState() {
  return (
    <div className="w-full max-w-2xl mx-auto flex flex-col items-center justify-center py-24">
      ...
      <p className="text-sm mt-1" style={{ color: C_SEC }}>Scoring agents against your criteria</p>
    </div>
  );
}
...
export default function BuyerResultsPage() {
  const [answers, setAnswers]     = useState<BuyerAnswers | null>(null);
  const [ranked,  setRanked]      = useState<MockAgent[]>([]);
  const [loading, setLoading]     = useState(true);

  useEffect(() => {
    const raw = typeof window !== 'undefined' ? localStorage.getItem('buyerAnswers') : null;
    const parsed: BuyerAnswers | null = raw ? JSON.parse(raw) : null;
    setAnswers(parsed);

    // Score and rank agents
    const scored = AGENTS.map(a => ({ agent: a, score: scoreAgent(a, parsed ?? { locations: [], budget: '', propTypes: [], timeline: '', financing: '', priorities: [], contact: { name: '', email: '', phone: '' } }) }));
  ...
  return (
    <div className="min-h-screen flex flex-col" style={{ backgroundColor: '#0F1117' }}>
      ...
      <nav className="max-w-7xl mx-auto w-full px-5 md:px-8 py-5 flex items-center gap-4">
        <Link href="/match/buyer" className="flex items-center gap-2 text-sm font-medium" style={{ color: C_SEC }}>
        ...
        {loading ? <LoadingState /> : (
          <>
            ...
            <div className="mb-8">
              ...
              {answers && (
                <p className="text-sm leading-relaxed" style={{ color: C_SEC }}>
                  Matched for{answers.locations.length > 0 ? ` ${answers.locations.slice(0, 2).join(', ')}` : ' Sonoma County'}
            ...
            <div className="flex flex-col gap-5 mb-8">
              {ranked.map((agent, i) => (
                <AgentCard key={agent.id} agent={agent} rank={i} answers={answers ?? { locations: [], budget: '', propTypes: [], timeline: '', financing: '', priorities: [], contact: { name: '', email: '', phone: '' } }} />
              ))}
            </div>
